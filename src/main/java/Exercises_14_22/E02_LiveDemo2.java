    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        do {
            List<WebElement> rows = driver.findElements(By.xpath("//tr/td[1]"));
             prices = rows.stream().filter(s -> s.getText().contains("Rice")).map(s -> getPriceVegetable(s)).collect(Collectors.toList());
            prices.forEach(a -> System.out.println("Price of Rice--> " + a));
